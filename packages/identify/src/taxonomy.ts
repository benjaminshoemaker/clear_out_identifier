...
export const taxonomy: CategoryEntry[] = [
...
export const brandLexicon: Record<string, string> = {
  sony: 'Sony',
  samsung: 'Samsung',
  apple: 'Apple',
...
export function normalizeBrand(text: string | undefined): string | undefined {
  ...
  for (const k of Object.keys(brandLexicon)) {
    if (key.startsWith(k)) return brandLexicon[k];
  }
  return text;
}

export function canonicalCategory(hint?: string): string {
  if (!hint) return 'misc';
  const t = hint.toLowerCase();
  for (const c of taxonomy) {
    if (c.id === t) return c.id;
    if (c.synonyms.some((s) => t.includes(s))) return c.id;
  }
  return 'misc';
}
