...
export function fuse(inputs) {
    ...
    let score = 0;
    if (inputs.codes.length)
        score += weights.w_code;
    if (model)
        score += weights.w_model * 0.8;
    if (brand)
        score += weights.w_brand * 0.7;
    if (inputs.clipNeighbors.length)
        score += weights.w_clip * Math.max(...inputs.clipNeighbors.map(n => n.score));
    if (inputs.vlm.category)
        score += weights.w_vlm * 0.6;
    if (inputs.ocrLines.length)
        score += weights.w_ocr_text * 0.5;
