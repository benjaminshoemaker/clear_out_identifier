...
export function detectHazards(text) {
    ...
    if (/(lithium|li-ion|nimh|battery)/i.test(lower))
        hazards.push('battery');
    if (/(aerosol|propane|butane|co2|pressur)/i.test(lower))
        hazards.push('aerosol');
    if (/(flammable|corrosive|acid|alkali|chemical)/i.test(lower))
        hazards.push('chemical');
    if (/(blade|knife|razor|cutter)/i.test(lower))
        hazards.push('blade');
    if (/(pressur|compressed)/i.test(lower))
        hazards.push('pressurized');
