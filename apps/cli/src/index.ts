...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .command(
      'analyze <images...>',
      'Analyze household item from images',
      (y: any) => y
      ...
      async (args: any) => {
        const files = (args.images as string[]).map((p) => path.resolve(String(p)));
        ...
        const enableStages = String(args.enableStages).split(',').reduce((acc: any, k: string) => { acc[k.trim()] = true; return acc; }, {});
        ...
        if (args.verbose) {
          ...
          const codes: string[] = (res as any).evidence?.codes || [];
          const mapped = codes.map((c: string) => ({ code: c, category: mapBarcodeToCategory(c) || null }));
          ...
          const lines = ((res as any).evidence?.ocr || []).slice(0, Number(args.maxOcrLines));
          if (lines.length) {
            console.log('OCR lines:');
            for (const l of lines) console.log(`  • ${l.replace(/\s+/g,' ').trim()}`);
          }
          if (rule.signals?.length) console.log(`Rule signals: ${rule.signals.join(' | ')}`);
        } else {
    ...
    .command(
      ...
      async (args: any) => {
        ...
        catch { console.log(`No manifest found at ${manifestPath}. Nothing to do.`); return; }
        const items: any[] = Array.isArray(m) ? m : (m.items || []);
        ...
        for (let idx = 0; idx < normItems.length; idx++) {
          ...
          const imgs: string[] = (it.images || []).map((p: string) => path.resolve(userDir, p));
          const bufs = imgs.map((f: string) => { try { return fs.readFileSync(f); } catch { return Buffer.from(`dummy-${f}`); } });
          const enableStages = String(args.enableStages).split(',').reduce((acc: any, k: string) => { acc[k.trim()] = true; return acc; }, {});
          const res = await analyzeItem(bufs, {
          ...
          const predCat = rule.category || (res as any).attributes?.category || (res as any).category || '';
          ...
          if (args.verbose) {
            ...
            const hazGT: string[] = Array.isArray(gt.hazards) ? gt.hazards : [];
            ...
            const neigh: Array<{id:string, score:number}> = (res as any).evidence?.neighbors || [];
            if (neigh.length) {
              console.log('Neighbors:');
              for (const n of neigh.slice(0,5)) console.log(`  • ${n.id} (${n.score.toFixed(2)})`);
            }
          } else {
            const line = `${it.id} | ${predCat} | ${gtCat} | ${res.resolution_level} | ${res.confidence.toFixed(2)} | ${brand}`;
            console.log(line);
          }
          if (predCat && gtCat && predCat === gtCat) correct++;
          total++;
